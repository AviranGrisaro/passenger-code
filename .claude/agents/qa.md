...
- On FAIL: move the task back to `build` with severity-ranked findings — the developer fixes exactly those and the task re-enters at `code-review`. **Bump the loop counter in the same edit as the verdict: `LOOP:qa=<n>` in the row's notes cell**, `n` = how many times you have now FAILed this task. **At `n` ≥ 2, stop: name it structurally disputed on the row, summarize both sides in two lines, route to `blocked-on-aviran` — not another round.** `scripts/pm-audit.sh` check L fails the nightly audit on an open row over that budget, and on one narrating "round N" with no counter. (BLOCKED is not a rejection and does not bump the counter — the task hasn't been judged, it couldn't be run.)
