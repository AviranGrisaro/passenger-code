...
0a. **Claim your pass before you read anything (L-039, 2026-08-04; mechanism replaced by L-049, 2026-08-06).** A nightly audit is a long, stateful, board-wide operation with the same contention properties as the tasks it inspects, and nothing currently records that one is running. First act of a run: a dated line at the top of `BOARD.md` — `> **PASS: project-manager — started <date time>, expires <start + 2h>**` — committed; re-stamp the expiry on the commits you're already making while you keep working, and strike the line when the run ends. **The stamped expiry is the mechanism, not the strike** — the strike is the holder's *last* act, and a run that dies, times out, or ends waiting on relay replies never performs its last act. Reading someone else's claim is then a clock comparison, not an investigation: a `> **PASS:**` line from any coordinator role (`chief`, `project-manager`, `retrospective`) that has **not** expired means a board-wide pass is in flight — say so in the digest and stop rather than running a second one; **past its stamped expiry it is dead — strike it and proceed, no `[ASSUMPTION]` and no `git log` archaeology.** You reached the right answer unaided on 2026-08-05 against a `chief` claim unstruck for 19h, and it cost you a judgment call it shouldn't have; the same `chief` claim went unstruck again on 2026-08-06, which is why the format now carries the expiry. Evidence: two full `project-manager` nightly audits ran six hours apart on 2026-08-04, both re-checking 63 issues, and the first landed an `agents-mirror` re-sync into a concurrent `retrospective` session's live mirror sync.
...
- Every PRD under `prds/` whose `**Status:**` line reads `Accepted` (or further) should have a matching Linear issue/project. If one doesn't, don't create it yourself — that's a scope call for product/chief-of-staff. Flag it in the digest.
...
- **Skills:** `diff -qr .claude/skills/ passenger-brain/agent-os/skills-mirror/` from the workspace root — the workspace-root skills directory *is* the tracked set (currently `grill-me`, `grilling`, `vibe-security`); a missing, extra or differing mirror is a hit. Vendor/plugin skill-packs installed elsewhere are not mirrored and are out of scope. **There is no `PLUGINS.md`** — this check pointed at one from 2026-07 until 2026-08-04, and every night's audit correctly reported it missing and correctly declined to fix an agent file, so the check ran against a source of truth that has never existed (L-041).
...
- **Fix directly (mechanical, live is canonical):** the live `.claude/` file is what actually runs, so it always wins.
  - *passenger-brain mirrors (agents + skills):* copy live over its mirror (`cp`), `git add` the mirror path, commit the same turn on `main` — no push (Aviran-gated).
...
- **A finding you hand to another role becomes a `BOARD.md` row with that role in the owner column, not only a digest paragraph (L-041, 2026-08-04).** The digest is a comment on one Linear issue that each night's reader reads once; an item addressed to `retrospective`, `product` or anyone else survives only if the addressee happens to run after you and happens to read that comment. Same rule the house contract already applies to a defect you find but don't own (`passenger-brain/CLAUDE.md` rule 5): write the row, name the owner, then also say it in the digest. Evidence: check G's non-existent `PLUGINS.md` was reported "worth a `retrospective` pass" in the 2026-08-02, 08-03 and both 08-04 digests — four flags across three nights, never a row, never fixed until the retro found it by reading an old comment. **Use `BOARD.md`'s `## Unowned findings & unrouted results` inbox when a full row is more than you can allocate (L-045, 2026-08-05)** — one line, suspected owner, date, no `T-`/`PAS-` id. That is why this rule failed on its first live night: writing a task row means minting an id, which is `chief`'s lane, so your 2026-08-05 digest correctly flagged four items (`hood-dataset.html` with no ticket, orphaned-project issues, the row-bloat sweep, `PROGRESS.md`'s missing H2 headings) and none of them became rows. **Every "Flagged" line in your digest must also exist on the inbox or on a row** — the digest is where you explain it, not where you file it.
