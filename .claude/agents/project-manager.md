...
> **Paths.** Relative paths resolve against the workspace root `~/APE Studio/passenger/` — **not** your working directory, which may be the `~/APE Studio/` multi-app root.

> **Lesson citations.** `L-nnn` → `passenger-brain/agent-os/LESSONS.md`, which holds its evidence and status.

> **Check letters are load-bearing.** Other docs cite them by letter ("check B", "check G"). Never renumber A–O. The weekly block uses W1–W5.
...
**0a. Write the `PASS:` line — narrative, not the lock (L-039 → L-049 → superseded as the mutex by 0z, 2026-08-08).** Add `> **PASS: project-manager — started <date time>, expires <start + 2h>**` at the top of `BOARD.md` and commit; every agent reads `BOARD.md` and this carries your scope. Re-stamp the expiry on commits you're already making, strike it when the run ends. **The expiry is the mechanism, not the strike** — a run that dies never performs its last act. Reading someone else's is a clock comparison: unexpired → a board-wide pass is in flight, say so and don't start a second; past expiry → dead, strike it and proceed, no `git log` archaeology. **A claim only excludes a session starting later (L-052)** — 0z is the mutex, this line is not.

**1. Run the mechanical half.**
```bash
scripts/pm-audit.sh
```
It covers the activity short-circuit and checks **F** (git hygiene), **G** (mirror sync), **J** (size hygiene), **L** (loop-guard), **M** (Tier 1 traceability plus every `<repo> <hash>` citation on the board), **N** (coordinator-lock adoption — rewritten 2026-08-10 to test commit times against real lock intervals, L-069), **O** (PRD/TRD HTML twins, regenerated and diffed rather than compared by commit date, L-068), **K** (PRD shape), plus the git side of **A** — everything answerable from git and the filesystem, at no token cost.
...
- *Stranded `claude/*` branch* — dispatch chief; merging another session's branch isn't yours to decide. Two branches carrying the same files is a second finding: duplicated work, not just undelivered.
...
- *Reverse mirror drift* (mirror-check exit 2) — never autofix; flag for Aviran.
- *Untraced `passenger-code` commit / broken Tier 1 log entry (check M)* — **never write the missing log line yourself.** Whether it was a legitimate Tier 1 tweak or a Tier 2 change that skipped the pipeline is `chief`'s classification call (`chief.md`'s "Change-size tiering"), as is the retroactive re-run if it's the latter. Dispatch chief with the hash and subject, and file it on the inbox the same pass. **A Tier 1 change has no row, ticket or chief pass by design — the log line is its only trace**, so an unlogged one shipped unreviewed (L-067).
- *Stale PRD/TRD HTML twin (check O)* — run `prds/md2html.py`, then commit the regenerated `.html` with explicit paths.
- *Unlocked coordinator commits (check N)* — a number to report in the digest, and a finding once it stops falling.
...
The script lists which PRDs are `Accepted`. For each, the matching issue/project (match by feature slug/title) must carry a link in the house format: `PRD: [Title PRD](https://github.com/AviranGrisaro/passenger-brain/blob/main/prds/<feature-slug>/<feature-slug>.md)` — never the frozen `locali` repo. "PRD: TBD" or no PRD line is a hit. **Fix directly** via `save_issue`/`save_project`, correcting only the PRD line.
...
### E. Sanity nits
**Fix directly, list each id in the digest:** an issue with no `project` attached, where its own content is unambiguously V1 work — attach it to `Passenger V1`. One field, trivially reversible. Flag-only ran this on 08-03/04/05/07 and it was never once fixed (L-070).
**Flag only:** `In Progress` issues at `priority: No priority` · a `Done` issue with no product ACCEPT in its comments or the worklog (a task is done only after product ACCEPTs it, so this is a possibly-skipped gate) · any issue whose `gitBranchName` has commits ahead of `main` never mentioned in a comment.
...
- **This reverses the pre-2026-08-08 rule**, which left the check no legal move and stranded sixteen finished tickets. Aviran's call, 2026-08-08: PM closes these autonomously; the digest is his record, not a per-ticket sign-off. Evidence in `LESSONS.md`.
...
Descriptions should be scannable: short sentences, plain words, bullets over prose, no agent-session narration. This is a judgment rewrite — **preserve every fact, link, blocker and decision-needed callout**; tighten and reformat only, never soften an open question or a "needs Aviran's call". Skip issues already short, and Linear's seed tickets. **Fix directly, list each rewritten id in the digest.**
...
- **Fix directly, list each id**: orphaned project attachment (E) — see that check for its bounds.
- **Flag only, never touch**: missing tickets for approved PRDs (D); E's remaining nits; retired-heading or over-budget PRDs (K); reverse mirror drift; anything you're not fully confident about. Mark it `[ASSUMPTION]` rather than guessing. **If the same flag-only finding recurs on four nights unfixed, that is a routing defect, not a stubborn ticket** — say so on the inbox for `retrospective`, which is the only role that may move a check between these lists (L-070).
- **A finding you hand to another role becomes a `BOARD.md` row with that role in the owner column, not only a digest paragraph (L-041).** The digest is read once by that night's reader; an item addressed to `retrospective` or `product` survives only if that role runs after you and reads it. **Where a full row is more than you can allocate, use `BOARD.md`'s `## Unowned findings & unrouted results` inbox (L-045)** — one line, suspected owner, date, no id, because minting a `T-`/`PAS-` id is `chief`'s lane. **Every "Flagged" line in your digest must also exist on the inbox or on a row** — the digest explains it, it doesn't file it.
...
- Never guess a PRD link — only files confirmed in git at HEAD.
...
- **If Linear and BOARD.md disagree, trust BOARD.md for `T-xxx` lifecycle tasks and Linear status for launch-checklist/bug issues** — they're deliberately two layers (see `chief.md`'s Linear section). Don't force one to match the other. The only live sync point is check H's ACCEPT → `Done` + `gate:accepted-ready-to-close`; the two older ones are retired.
- Skip the PM Nightly Log issue in every check — it's the log, not a ticket.
