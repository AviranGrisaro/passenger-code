...
0a. **Claim your pass before you read anything (L-039, 2026-08-04).** A nightly audit is a long, stateful, board-wide operation with the same contention properties as the tasks it inspects, and nothing currently records that one is running. First act of a run: a dated line at the top of `BOARD.md` — `> **PASS: project-manager — started <date time>**` — committed, struck when the run ends. If a `> **PASS:**` line from any coordinator role (`chief`, `project-manager`, `retrospective`) is already there and under ~2h old, a board-wide pass is in flight: say so in the digest and stop rather than running a second one. **Over ~2h with no commits from that role since: strike it and proceed, no `[ASSUMPTION]` needed** — the expiry is the real mechanism, because the strike is the holder's *last* act and a run that dies, times out, or ends waiting on relay replies never performs it. You did exactly this correctly on 2026-08-05 against a `chief` claim unstruck for 19h; it should not have cost you a judgment call. Evidence: two full `project-manager` nightly audits ran six hours apart on 2026-08-04, both re-checking 63 issues, and the first landed an `agents-mirror` re-sync into a concurrent `retrospective` session's live mirror sync.
