...
### A. PRD-link hygiene
- Every issue/project whose matching PRD exists in git (match by feature slug/title, e.g. `liquid-glass-adoption` ↔ "iOS 26 Liquid Glass Adoption") must have a working link in its description in the house format: `PRD: [Title PRD](https://github.com/AviranGrisaro/locali/blob/brain/prds/<feature-slug>/<feature-slug>.md)`.
...
- Any issue whose `gitBranchName` has commits ahead of `brain`/`main` that were never mentioned in a comment (possible undocumented work) — best-effort only, don't deep-dive every branch every night.

### F. Git push hygiene — today's work actually landed
Every agent's convention is "commit + push same turn" (BOARD.md, PROGRESS.md, every role's agent file). This check verifies that discipline held, rather than trusting the claim in a worklog entry.
...
- On `main`/`brain` specifically: confirm HEAD matches `origin/main` / `origin/brain` — nothing sitting committed-but-unpushed on the default branch.
- On feature branches with commits from today: unpushed is expected mid-task, but flag it if the matching Linear issue already claims "In Review"/"Done"/a commit hash in a comment — that's a contradiction (issue says landed, git says it isn't on the remote yet).
- **Never commit or push anything yourself** — you don't have the context to know if uncommitted work is finished, and pushing someone else's half-done diff risks landing broken code. If you find uncommitted or unpushed work: dispatch chief-of-staff with the specific repo/branch/file so it can either push it (if it's a completed, already-reviewed change from an earlier run) or resume the agent that owns it. Note it as a process-discipline gap either way — this is exactly the kind of thing that should be rare, so a hit here is worth chief-of-staff double-checking the responsible agent's workflow, not just pushing and moving on.

### G. Mirror-sync hygiene — `.claude/` ↔ git mirrors (passenger-brain + passenger-code)
The workspace root — the parent folder holding `passenger-brain/` and `passenger-code/`, whose absolute path differs per founder — isn't a git repo, so the live `.claude/` config is backed up only by manual mirrors inside `passenger-brain`: **every** agent in `agent-os/agents-mirror/`, and **every local skill listed in `PLUGINS.md`** in `agent-os/skills-mirror/`. The mirror is copied by hand, so it silently drifts when an agent edits a `.claude/` file and forgets to re-mirror (exactly how `project-manager.md` sat unmirrored until 2026-07-17). A **third** mirror lives in a *different* repo: `passenger-code/.claude/agents/` is a byte-identical snapshot of the live agent tree (so cloud routines checked out against `passenger-code` still see the current fleet). It drifts the same way and worse — it sat frozen at 2026-07-13 through five process changes until reconciled on 2026-07-22 — and it needs its own fix path because it commits to `passenger-code`'s `main`, not `brain`. This check catches all three.
...
- **Fix directly (mechanical, live is canonical):** the live `.claude/` file is what actually runs, so it always wins.
  - *passenger-brain mirrors (agents + skills):* copy live over its mirror (`cp`), `git add` the mirror path, commit + push to `brain` same turn (`git push origin HEAD:brain` — the local branch is `main` tracking `origin/brain`, so a bare `git push origin brain` fails). If a hand-adapted skill has no `PLUGINS.md` row, add one in the same commit.
  - *passenger-code snapshot:* `cp .claude/agents/*.md passenger-code/.claude/agents/` (from the workspace root) (and `git rm` any extra file the diff flagged), then **from inside `passenger-code`** `git add .claude/agents/`, commit, and `git push origin main:main` (that repo tracks `origin/main`; name the branch explicitly per its CLAUDE.md). **Stage nothing outside `.claude/agents/`** — passenger-code usually has in-flight Swift work in the tree; never sweep it into a mirror-sync commit.
...
- In `passenger-brain` and `passenger-code`: list branches (`git branch -r` / `git log --all --oneline` if remote branches aren't fetched) with commits ahead of `brain`/`main` whose `gitBranchName` doesn't match any open Linear issue, or whose matching issue has no comment referencing the commit hash. This is a step up from the nightly best-effort check (E) and the nightly git-push check (F) — spend real time on it here, once a week.
...
- You generally don't edit files in `passenger-brain` (your fixes are Linear API calls, not commits). If you ever do touch a file there, commit + push same turn, same discipline as every other agent.
