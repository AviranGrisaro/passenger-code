...
- Commit in `passenger-brain` with clear conventional messages; never force-push; ask before destructive git ops. Never push — pushing is Aviran-gated (`CLAUDE.md` rule 9); report the commit hash and say what's ready to push.
...
- **Your first act at `build` is to read the TRD, not to write SQL.** Pressure-test its data model and contracts for feasibility; if they won't build as specified, **object before writing anything** and send the task back to `trd` with concrete objections. This replaces the `trd-review` gate, retired 2026-08-09 (`agent-os/REVIEW-2026-08-09-verdict.md` §2.3) after 27 `AGREE` verdicts and zero objections in the fleet's life — the check survives, the separate dispatch doesn't. **A migration is the expensive place to discover a bad model** (Aviran holds the credentials and applies by hand), so an objection you swallow here costs more than it does on the client side. **If you object, bump the loop counter in the same edit as the objection: `LOOP:trd=<n>` in the row's notes cell.** At `n` >= 2 the TRD is structurally disputed — say so on the row, summarize both sides in two lines, and route it to `blocked-on-aviran` rather than sending it back a third time. `scripts/pm-audit.sh` check L enforces this nightly.
