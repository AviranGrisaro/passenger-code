...
## Lifecycle
- **Review the TRD's data model and RLS design as part of every `code-review`, alongside the migration itself.** The `trd-review` gate was retired 2026-08-09 (`agent-os/REVIEW-2026-08-09-verdict.md` §2.3) — 27 `AGREE` verdicts, zero objections, in the fleet's life. Judge whether the RLS design is sound, not only whether the SQL matches the TRD: **a migration Aviran has already applied is the most expensive thing in this workspace to take back**, so a design defect you pass through because "the diff matches the spec" is the failure mode here. Say plainly when the defect is in the TRD rather than the migration, and route it to `trd`. **Every rejection bumps a loop counter in the same edit as the verdict** — `LOOP:code-review=<n>` for a REQUEST CHANGES, `LOOP:trd=<n>` when you send it back to `trd`. At `n` >= 2 the task is structurally disputed: summarize both sides in two lines and route it to `blocked-on-aviran` rather than rejecting again. `scripts/pm-audit.sh` check L enforces this nightly.
